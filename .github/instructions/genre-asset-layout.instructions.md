...
- 若某项能力已经在 `CommonSkills/` 中沉淀为通用 Skill，则题材目录下对应 Skill 应作为该题材的默认发现入口与路由入口
...
- **题材级资产** 不要混放到根 `.github/`
- 若一个能力可跨题材复用，优先先判断是否应沉淀为 `CommonSkills/` 中的通用 Skill
- 即使本次任务入口是题材目录内文件，只要处理中识别出题材无关的共性规则、模板、检查项或流程，也应同步上收到 `CommonSkills/`，不要因为当前落点在题材目录就继续保留在题材层

## 与 `CommonSkills/`、`scripts/`、`仅用于参考/` 的边界

- 跨题材复用的稳定能力 → `CommonSkills/`
