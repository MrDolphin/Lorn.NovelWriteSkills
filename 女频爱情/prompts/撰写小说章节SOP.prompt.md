...
命中本 Prompt 时，必须先调用：`通用-章节创作闭环`。

若当前批次章节已存在明确卷纲 / 章清单，默认应把 `通用-生成章节控制卡` 视为正文施工前的上游必经层。
...
这份 Prompt 现在是**女频爱情入口层**；真正的章节创作 SOP、批量模式、日志机制与题材护栏，都必须继续到对应 `通用-*` Skill 中读取并执行。
