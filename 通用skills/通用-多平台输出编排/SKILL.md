...
- 在任何平台分发开始前，必须先为**每个章节 × 每个平台**建立字数可行性判断；正文与 `## 作者有话说` 未达到该平台的最低字数门槛时，**不得**进入平台正文写作阶段。
- 若源稿本身偏短，必须先调用 `通用-提纯多平台母稿` 后判断是否需要做源稿补强；若源稿补强后仍无法达到目标平台的硬门槛，必须停止该平台分支，记录 `blocked_word_count_insufficient`，不得用平台后处理“凑字数”。
- 字数不足的处理顺序必须是：**先补信息与场景 → 再补正文 → 再补作者有话说 → 最后才允许进入平台门禁与相似度终检**。禁止把字数不足留到平台分支里临时修补。
- 平台化草图阶段必须显式写明该平台的字数余量判断：
	- 中文平台：正文目标 3500–6500 CJK，作者有话说 200–300 CJK；
	- 英文平台：正文目标 >= 6500 words/len，作者有话说 100–150 words。
- 若任一平台版本在首次落盘后仍未达字数门槛，必须立即回炉扩写，不得先保留“能跑门禁”的短稿再后补。
...
- 不得把“先落盘、再补字数”作为流程策略；这会让平台门禁、相似度门禁和 POV 门禁都建立在错误长度上。
- 不得把短稿先分发到所有平台，再期待某个平台的作者有话说或平台后修补到位。
- 若当前章节在某平台上无法稳定达到字数门槛，必须在日志中直接标记该平台分支为 `blocked_word_count_insufficient`，并中止后续平台分发。
- 字数门槛是前置门槛，不是终检补项；它与 POV 契约门禁同级，不能放到相似度之后处理。
