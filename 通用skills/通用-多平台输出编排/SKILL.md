...
argument-hint: '请给我一个 chapterPath 或 chapterPaths；platforms 可以省略。若用户命令中已指定 platforms，按指定子集执行；若未指定，必须读取项目根目录 `平台POV基线表` 获取默认输出平台列表，并读取 `Agents.md` 识别主输出平台（主平台版本已由 `通用-创建小说正文` 写入 `小说正文/`，本编排器对主平台仅执行门禁终检，不重复改写）；若 `平台POV基线表` 不存在，提示用户创建后停止。'
---
...
4. 平台参数解析（三步优先级）：<br>
	...
	`平台POV基线表` 格式要求：每行一个平台名（与 `通用-输出*版` Skill 名称后缀一致），`#` 开头的行为注释。<br>
	④ 同时读取项目根目录 `Agents.md`（若存在）识别**主输出平台**；主平台版本已由 `通用-创建小说正文` 写入 `小说正文/`，本编排器对主平台**只执行门禁终检**（字数、POV、相似度），**不重复做平台改写**。<br>
