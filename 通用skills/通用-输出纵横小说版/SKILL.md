---
name: 通用-输出纵横小说版
description: '用于把小说章节改写为更适合纵横小说网的版本。适合中等节奏、现实锚点扎实、慢热逻辑推进、人性落点显化与克制悬念渲染的正文输出；供多个题材的 `题材名-输出纵横小说版` 包装层路由使用。关键词：输出纵横小说版、纵横中文网、中等节奏改写、男频悬疑平台。'
argument-hint: '要把哪一章输出为纵横小说版？默认按"保留核心事件链 + 中等节奏克制悬念 + 现实锚点强化 + 人性落点显化"处理。'
---
...
4. 若项目根目录不存在 `Agents.md`，或其中未注册纵横专属模板——回退通用默认模式，不影响正常输出
5. 本 Skill 的核心约束始终是：平台规则 > 安全合规 > 母稿事实 > 三模板约束。三模板是辅助，不改变平台输出规则

## 默认执行顺序

1. 先锁定源稿不可变层：事实、事件链、证据链、角色关系与因果顺序不得改坏。
2. 读取对应题材的 `题材名-输出纵横小说版` Skill，确认题材入口与路由关系。
3. 再读取本 Skill 的 `references/平台共性执行细则.md` 与 `references/分节级补救映射与详细规则回填.md`，执行纵横平台的中等节奏调整、克制悬念渲染、现实锚点强化与人性落点显化。
4. 由题材包装层继续读取其题材 references，锁定题材边界、题材禁行项与题材专项补丁（作为兜底约束）。
5. 在“平台优先、题材兜底”前提下执行改写正文，确保平台化表达与源稿保真同时成立。
6. 最后才处理降相似度，不得为了降重破坏中速节奏、现实锚点与人性落点。
...
**为什么纵横两种人称都可行？**
- 纵横读者对"同步推理"的需求高于一切。无论是第一人称还是第三人称，关键约束是**感知边界不可突破**——读者获取的信息必须=主角获取的信息。上帝视角是唯一死刑。
...
**判断标准——回答两个问题**：
1. "主角有没有'失去意识/无法感知'的关键场景？"——如果有，第三人称给了你补信息的合法通道。
2. "故事最大的悬念，是让读者解谜，还是让读者共情？"——前者偏第三人称（信息拼图需要多视角），后者偏第一人称（恐惧和判断的即时传递最有力）。
...
0. **字数达标**：正文和 `## 作者有话说` 必须先满足本 Skill 的字数门禁；
  - 正文目标区间为 **3500–6500 CJK 字符**，硬下限 3500，硬上限 6500。
  - `## 作者有话说` 目标区间为 **220–280 CJK 字符**，硬下限 200。
  - 正文未达硬下限前，不得进入后续自检，必须先回炉补足字数。
...
- 落盘路径的完整推断优先级：① 用户显式指定 chapterPath → ② 从源文件名与分部/卷信息自动推断 → ③ 若无法确定分部/卷，暂停并向用户确认，不得乱推断后静默落盘。
 ...
 - 生成阶段必须先达到字数保底目标再允许落盘：正文目标 **3500–6500** CJK（硬下限 `3500`），`## 作者有话说` 目标 `220–280` CJK（硬下限 `200`）；未达保底目标时必须先回炉扩写，禁止"先落盘再补字数"。
