...
0. **字数达标**：正文和 `## 作者有话说` 必须先满足本 Skill 的"平台默认字数范围"（或项目 `Agents.md` 中的覆盖值）；
...
- 平台稿落盘后必须显式运行字数门禁：`scripts/count-chapter.ps1`，参数取本 Skill 的"平台默认字数范围"或项目 `Agents.md` 中的覆盖值；并运行 `scripts/count-afterword.ps1`，参数取对应作者有话说范围。
