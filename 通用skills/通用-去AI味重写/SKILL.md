...
- 去味完成后必须补回真实个性：立场、矛盾、毛边、偏见与选择性观察。- **深度诊断层**（见下文 `## AI味深度诊断`）：当常规去味后文本仍然"读着像AI但挑不出具体毛病"时，进入九大结构指纹扫描——前六项覆盖文本结构层（感官均匀轰炸、比喻公式套娃、节奏全程匀速、内心独白过度条理、元叙事总结口吻、职业设定教科书执行），后三项覆盖创作逻辑层（模板机械复现、术语历史嫁接、过度合理缺失盲区），辅以四项量化检测指标（破折号异常率、句长标准差、感官词密度、转折扣密度）。这些是高级AI润色/扩写后的深层痕迹，常规模板腔检查无法覆盖。
...
- `references/AI句式替换与动作化替代清单.md`（**必读** — C批次高发AI句式替换）
- `references/去AI味执行清单.md`
- `references/去AI味红线保护.md`
- `references/去AI味句级改写工具.md`
- `references/去AI味识别雷达.md`
- `references/去AI味共享裁判规则补编.md`（**必读** — 整编外部 `deai-rules` 的共性识别口径、小说专项病灶与收尾快检）
...
- `references/AI味高发信号与拆洗策略补充.md`（表层信号——句子均匀、对话同腔、抽象判断等常规病灶）
> **注意**：当常规模板腔检查通过后文本仍然"读着像AI"时，不要重复跑本文件中的常规扫描，应进入本 Skill 正文的 `## AI味深度诊断` 节，启动六大结构指纹扫描。表层信号和深层指纹是互补关系——前者抓"句子像AI"，后者抓"整章有AI深度参与"。两者必须分层执行，不得混淆。
...
- `references/风格注入锚点卡.md`（**有作者风格模板时必读** — 人声回补层从通用"补毛边"升级为按作者风格模板定向注入）
...
- **缓存层（永久不变）**：frontmatter + 外部共享规则吸收口径 + 五大扫描面定义 + 四维病灶标准 + 九大结构指纹诊断 + 四项量化检测指标 + 三档手术强度 + 红线保护规则 → 每次调用完全相同，按底价计费
- **项目层（同项目内不变）**：references 清单 + 风格/蓝本/写作研究模板自动发现规则 + 主输出平台自动发现规则 → 仅在切换项目、平台或新增模板注册时变化
- **可变层（每次变化，不写入本文件）**：本次目标章节文件路径、去味强度档位、本次特殊约束 → 由用户每次调用时指定
- 人物传记、技术设定等裁判源 + 作者风格模板通过 references 声明强制加载，其固定部分随本 Skill 一起进入缓存前缀
...
- 动手去 AI 味前，必须认真仔细阅读与本章实际出场人物对应的人物传记，以及与本章背景、技术、规则、证据、程序链、世界观有关的技术设定文档。
- 不得只凭“这一段很像 AI”就直接拆句；凡涉及人物动机、能力边界、关系状态、设定代价、证据机制的句段，都必须回到作者侧裁判源核对后再改。
- 若同一人物传记或技术设定存在多个版本，必须先确认当前可执行的最新版本；若关键裁判源缺失，必须先补齐、定位或明确阻断，不能边猜边去味。
...
1. 先认真仔细阅读本章相关人物传记、技术设定 / 背景设定、控制卡与最新审阅结论，确认不可改动的钉子、必须保住的声口与不能洗掉的规则边界。
2. 再确认处理范围、题材、不可改动的钉子，以及用户是否已指定强度。
3. 先用“四维病灶”做一句话初诊：习惯用语、句式逻辑、写法口气、论证逻辑分别哪里最假。
4. 若用户未定强度，先展示“三档手术强度与风险卡”，再锁定本轮只采用的强度等级。
5. 先做内容与口径保护，再交叉使用“识别雷达”与“共享裁判规则补编”，定位套话、均匀句群、内容抬升腔、模糊归因、假专业句、假钩子、回合制打斗、章节自指导览腔、交流残留与解释腔。
6. 再按“45 条规则总表”“共享裁判规则补编”与“三级改造与工具包”处理语言、节奏、情绪、场景具体度、潜台词和代价纹理；默认同时执行“删填充、拆公式、变节奏、信任读者、删金句”五条共性原则。
7. 输出时必须按“多轮诊断与回写模板”给出：修改总结与原因 → 修改后的小说正文 → 后续修改建议。
8. 最后用“去AI味后供血与职责复检卡”复检卖点供血、章首抓力、中段回报、章末钩子与人物声音差是否仍成立。
...
- 先守信息与结构边界：
  ...
  - 中段回报与章末钩子不能被磨平。
  - 未认真仔细阅读本章相关人物传记与技术设定 / 背景设定文档前，不得直接改写人物声口、规则解释、证据逻辑、关系状态或代价结构。
- 再守去味表达边界：
  - 不得把正文洗成白水、散文或总结稿。
