---
name: 分发-起点中文网
description: 用于将 `起点中文网/` 目录内指定章节批量上传到起点中文网作者后台并保存为草稿。支持按 `起点中文网/README.md` 读取账号、密码、起点书籍Id、书名与卷名校验，自动执行登录、进入新建/修改章节页、填写合并标题输入框、粘贴正文、添加作家的话、保存草稿，并在保存后从左侧章节列表反查章节Id并写回 `起点中文网/分发记录.md`。关键词：起点上传、起点中文网、保存草稿、批量传章、作家的话、章节Id回写。
argument-hint: 给我 chapterPath 或 chapterPaths（位于 `起点中文网/` 目录），我会按 `起点中文网/README.md` 的账号与书籍信息逐章上传并保存草稿，同时回写章节Id。
---
...
2. 若当前平台（起点中文网）匹配 `主输出平台` → 工作目录为 `小说正文/`
3. 若当前平台不匹配 `主输出平台`，或 `Agents.md` 不存在/未声明 `主输出平台` → 工作目录为 `起点/`

后续所有章节文件路径、README.md、分发记录.md 均基于此工作目录。**明确说明**：
  - **本平台是主输出平台时** → 章节文件、`README.md`、`分发记录.md` 均在 `小说正文/` 下
  - **本平台不是主输出平台时** → 章节文件、`README.md`、`分发记录.md` 均在 `起点/` 下

> **路径说明**：以下各节中所有 `起点/` 形式的路经（如 `起点/README.md`、`起点/分发记录.md`）均为工作目录的示例写法。实际目录由上方「工作目录路由规则」确定：本平台是主输出平台时，所有这些路径对应 `小说正文/`；不是主输出平台时，对应 `起点/`。

## 何时使用

- 你已经把待上传章节放入 `起点/` 目录。
- 你已经在 `起点/README.md` 写好账号、密码、起点书籍Id、书名、卷名信息。
...
读取 `起点/README.md` 后，必须检查其第一行一级标题是否等于 `# 起点中文网分发配置`：
  - **匹配** → 继续执行后续流程
  - **不匹配** → 立即停止上传，并向用户报错：`README.md 平台标识与当前 Skill 不符：预期"起点中文网分发配置"，实际读取到"{实际读取到的标题}"。请检查是否将其他平台的 README.md 误放到了当前工作目录。`
### 3) 分发记录来源（强制）
...
1. 读取并解析 `起点/README.md`。
2. 确认已获得：账号、密码、目标书籍Id（按"书籍Id判定规则"选取）、目标小说名、目标卷名。
3. 读取 `起点/分发记录.md`，建立"章节唯一键 -> 章节Id"映射（建议唯一键：`起点书籍ID|分部名|卷号|章号|章名`；无分部时分部名固定为 `NA`）。
...
1. 在访问前，先查询 `起点/分发记录.md` 中该章节是否已有 `章节Id`。
...
1. 将"章节唯一键 + 章节Id + 更新时间"写回 `起点/分发记录.md`：
   - 若记录不存在：新增一条。
...
- 每章进入步骤 3 前，必须先查询 `起点/分发记录.md`。
- 新建模式必须使用 `#ccid=-1`，修改模式必须使用 `#ccid={章节Id}`，二者不得混用。
- 每章保存成功后，必须尝试从左侧章节列表或页面 hash 中拿到最终 `章节Id` 并回写 `起点/分发记录.md`。
...
- 书名/卷名不匹配：直接停止该书上传流程，要求先修正 `起点/README.md` 或目标书籍Id。
...
- `起点/分发记录.md` 写入失败：本章虽然可能已保存草稿，但流程状态必须标记为"部分失败（记录未落盘）"，并立即提示人工补录，禁止静默成功。
...
- `起点/分发记录.md` 使用 Markdown 表格时，`章节唯一键` 内部的分隔符 `|` 必须转义为 `\|`，否则会触发列数错乱（如 MD056）。

### 本轮实操补充（2026-05-21）

- 长批量中若出现“保存失败”弹窗（例如“请输入1~20000字以内的章节内容”），该弹窗会拦截后续点击；**必须先点“我知道了”清弹窗**，再继续下一章，否则会连锁超时失败。
- 标题框与“作家的话”输入框都可能包含“请输入”占位文案；自动化时应使用**更窄定位器**（标题框优先 `请输入章节号与章节名`，作者话优先右侧抽屉 `textarea[placeholder="请输入"]`），避免把作者话误填进标题框。
- 每章保存后建议立刻做“**三点回读**”：
   1) 标题框值是否为 `第{章号}章 {章节标题}`；
   2) 当前 URL 是否拿到有效 `#ccid=数字`；
   3) 左侧草稿列表是否出现对应标题。
   任一项不通过即判定“未闭环”，不得直接进入下一章。
- 遇到“无标题章节”污染时，先分流处理：
   - 若能精确定位到正确 `ccid`：进入该 `ccid` 修正标题/正文/作者话并保存；
   - 若章节内容已串写或无法确认归属：直接按新建重传该章，获取**新 `ccid`**，并以新 `ccid` 回写 `分发记录.md`。
- 批量结束后必须做“**章号-标题-ccid 对账**”（至少覆盖本批次范围），确认每个章号只有一个有效映射；历史错误/临时 `ccid` 不得写入最终 `分发记录.md`。
- 新建批量时，默认每章都从 `#ccid=-1` 起步，避免在异常草稿上连续覆盖导致串章；只有当 `分发记录.md` 已有明确映射时才进入修改模式。
...
1. **输入是否齐全**
   - 已明确本次只上传哪些章节（不扩范围）。
   - `起点/README.md` 可读到用户名、密码、书籍Id、书名、卷名。
...
- 已读取 `起点/README.md` 并提取必需字段。
- 已按指定章节列表逐章执行上传。
- 已在每章执行前查询 `起点/分发记录.md` 并正确选择新建/修改 URL。
- 每章均完成：标题输入框、正文、作家的话、保存草稿/保存。
- 每章保存后均已尝试并优先通过左侧章节列表获取 `章节Id`。
- 每章均已记录或更新到 `起点/分发记录.md`（含章节与章节Id映射）。
- 每章上传前均完成目标作品信息校验。
