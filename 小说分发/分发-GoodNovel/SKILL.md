...
2. 若当前平台（GoodNovel）匹配 `主输出平台` → 工作目录为 `小说正文/`
3. 若当前平台不匹配 `主输出平台`，或 `Agents.md` 不存在/未声明 `主输出平台` → 工作目录为 `GoodNovel/`

后续所有章节文件路径、README.md、分发记录.md 均基于此工作目录。**明确说明**：
  - **本平台是主输出平台时** → 章节文件、`README.md`、`分发记录.md` 均在 `小说正文/` 下
  - **本平台不是主输出平台时** → 章节文件、`README.md`、`分发记录.md` 均在 `GoodNovel/` 下

> **路径说明**：以下各节中所有 `GoodNovel/` 形式的路经（如 `GoodNovel/README.md`、`GoodNovel/分发记录.md`）均为工作目录的示例写法。实际目录由上方「工作目录路由规则」确定：本平台是主输出平台时，所有这些路径对应 `小说正文/`；不是主输出平台时，对应 `GoodNovel/`。
...
- 必须从章节文件中提取：章号（阿拉伯数字）、章节标题、正文。
- 若章节文件标题行为 `# 1.1.3 三分钟`，其中 `1.1.3` 表示 `部号.卷号.章号`；提取时必须只取最后一段 `3` 作为源章号，`三分钟` 作为“章节标题”。
- GoodNovel 最终上传时虽然使用绝对章号口径，但绝对章号只能基于最后一段章号再按本 Skill 的规则换算，不得把 `1.1.3`、`2.4.17` 这类三级编号原样带入标题输入框；最终只能写成 `Chapter 95 三分钟` 一类格式。
...
读取 `GoodNovel/README.md` 后，必须检查其第一行一级标题是否等于 `# GoodNovel 分发配置`：
  - **匹配** → 继续执行后续流程
  - **不匹配** → 立即停止上传，并向用户报错：`README.md 平台标识与当前 Skill 不符：预期"GoodNovel 分发配置"，实际读取到"{实际读取到的标题}"。请检查是否将其他平台的 README.md 误放到了当前工作目录。`
...
当 `GoodNovel/README.md` 不存在时，必须按以下模板创建：

```markdown
# GoodNovel 分发配置

> 用途：存储 GoodNovel 书籍ID与书名。分发流程需要用户手动登录，不读取密码。
...
- `GoodNovel 书籍ID`：从 GoodNovel 作者后台获取

## 登录方式

- **本文件不存储密码**。分发流程将在浏览器打开 GoodNovel，需要你在浏览器中**手动登录**。
- 确保登录账号拥有上述书籍的编辑权限。

## 注意事项

- 分发流程会检查登录状态；如未登录，需要你手动完成登录后再继续。
- GoodNovel 无作者有话说独立入口，相关内容无法单独提交。
- 章节ID获取依赖当前页面状态，可能存在回填延迟。
```
...
4. 读取你指定的章节文件，提取每章：源章号（最终一段章号）、章节标题、正文。
