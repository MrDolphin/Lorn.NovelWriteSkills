...
2. 若当前平台（新浪微博）匹配 `主输出平台` → 工作目录为 `小说正文/`
3. 若当前平台不匹配 `主输出平台`，或 `Agents.md` 不存在/未声明 `主输出平台` → 工作目录为 `新浪微博/`

后续所有章节文件路径、README.md、分发记录.md 均基于此工作目录。**明确说明**：
  - **本平台是主输出平台时** → 章节文件、`README.md`、`分发记录.md` 均在 `小说正文/` 下
  - **本平台不是主输出平台时** → 章节文件、`README.md`、`分发记录.md` 均在 `新浪微博/` 下

> **路径说明**：以下各节中所有 `新浪微博/` 形式的路经（如 `新浪微博/README.md`、`新浪微博/分发记录.md`）均为工作目录的示例写法。实际目录由上方「工作目录路由规则」确定：本平台是主输出平台时，所有这些路径对应 `小说正文/`；不是主输出平台时，对应 `新浪微博/`。
...
读取 `新浪微博/README.md` 后，必须检查其第一行一级标题是否等于 `# 新浪微博分发配置`：
  - **匹配** → 继续执行后续流程
  - **不匹配** → 立即停止上传，并向用户报错：`README.md 平台标识与当前 Skill 不符：预期"新浪微博分发配置"，实际读取到"{实际读取到的标题}"。请检查是否将其他平台的 README.md 误放到了当前工作目录。`
...
### 本次批量上传（第1卷 1-40章）复盘：经验

- 先做"离线预处理队列"再进浏览器，显著降低串章率：标题、绝对章号、导语（≤44字）、正文先在本地固化为同一批次数据源。
- 采用"小样本冒烟（先跑 1-5 章）→ 小批次推进（5-10章/批）"更稳，能在大规模执行前尽早暴露定位器与字段映射问题。
- 登录链路用"自动探测 + 人工兜底"更可靠：当登录页形态变化导致自动识别失败时，先人工确认登录态，再恢复自动批量执行。
- 每个小批次结束立即回写 `分发记录.md`，可以把回滚范围限制在当前批次，避免整批重做。
- 标题统一使用 `第{绝对章节号}章 {章节标题}` 后，再填正文与导语，能明显减少后期返修。

### 本次批量上传（第1卷 1-40章）复盘：教训

- 不要把"登录页结构稳定"当作前提：微博登录入口会出现不同形态，缺少登录态兜底会直接卡住整批流程。
- 不要把跨域本地服务当成唯一路径：当本地 CORS 服务不可用时，必须自动回退到本地文件读取/本地队列读取，不得中断批次。
- 不要跳过每章实时分流：即使已有预判结果，也要在执行前再次查询 `分发记录.md`，防止误把应修改章节当新建。
- 不要延后对账：每批必须立即核对 `计划章数 = 成功章数 + 失败章数`，并确认成功章均已写回 `章节Id`。
- 不要在失败章上死循环：单章重试超过阈值后应记录失败步骤与页面提示，先推进后续章节，最后回到失败清单补传。
