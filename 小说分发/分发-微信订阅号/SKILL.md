...
2. 若当前平台（微信订阅号）匹配 `主输出平台` → 工作目录为 `小说正文/`
3. 若当前平台不匹配 `主输出平台`，或 `Agents.md` 不存在/未声明 `主输出平台` → 工作目录为 `微信订阅号/`

后续所有章节文件路径、README.md、分发记录.md 均基于此工作目录。**明确说明**：
  - **本平台是主输出平台时** → 章节文件、`README.md`、`分发记录.md` 均在 `小说正文/` 下
  - **本平台不是主输出平台时** → 章节文件、`README.md`、`分发记录.md` 均在 `微信订阅号/` 下

> **路径说明**：以下各节中所有 `微信订阅号/` 形式的路经（如 `微信订阅号/README.md`、`微信订阅号/分发记录.md`）均为工作目录的示例写法。实际目录由上方「工作目录路由规则」确定：本平台是主输出平台时，所有这些路径对应 `小说正文/`；不是主输出平台时，对应 `微信订阅号/`。
...
- 必须将章节文本拆成两个独立字段：`正文` 与 `作者有话说`。
- 正文编辑区**禁止**出现 `## 作者有话说` 标题及其内容。
- “作者有话说”只能进入摘要/作者有话说输入框（并压缩至 120 字以内），不得回灌正文。
...
读取 `微信订阅号/README.md` 后，必须检查其第一行一级标题是否等于 `# 微信订阅号分发配置`：
  - **匹配** → 继续执行后续流程
  - **不匹配** → 立即停止上传，并向用户报错：`README.md 平台标识与当前 Skill 不符：预期"微信订阅号分发配置"，实际读取到"{实际读取到的标题}"。请检查是否将其他平台的 README.md 误放到了当前工作目录。`
### 3) 分发记录来源（强制）
...
4. 保存前必须校验标题字符串不含 `undefined` / `NaN` / 空章号。
...
- 点击后弹出浮层中：
  - “声明类型”选择“文字原创”；
   - “作者”填写 `微信订阅号/README.md` 中声明的作者笔名；**必须使用模拟键入（逐字输入）方式填写，不得仅粘贴或脚本一次性赋值**，以确保触发前端校验；若出现“字符数为0/仍提示需填写作者”，先聚焦输入框并清空，再以模拟键入重填；
  - 点击“确定”。
- 点击“赞赏”后面的链接，在弹出的对话框中：
   - “赞赏类型”选“赞赏作者”；
   - 然后点击“确认”。

#### 5.2.1 赞赏（必选，阻断级）

- 赞赏不是可选优化项，属于保存前阻断级门禁。
- 若未明确完成“赞赏作者”设置，不得进入“保存为草稿”步骤。
...
- 预处理产物必须包含最小数据模型（缺一不可）：
   - `absoluteChapterNo`
   - `chapterTitle`
   - `fullWechatTitle`（`第{绝对章节号}章 {章节标题}`）
   - `bodyText`
   - `afterwordText`
   - `afterwordSummary120`
   - `mode`（新建 / 修改）
   - `existingAppmsgId`（修改模式）
...
#### H. 2026-05-21 事故复盘硬化门禁（强制）

- 以下任一命中，当前章必须判定失败并阻断保存：
   - 标题出现 `第undefined章` 或章号为空；
   - 正文包含 `## 作者有话说` 标题块；
   - 赞赏未设置为“赞赏作者”；
   - 合集仍为“未添加”。
- 禁止把“拿到 `appmsgid`”作为质量通过条件；`appmsgid` 只代表保存成功，不代表内容与设置合格。
## 关键边界与硬规则
...
- “文章设置”四项必须完成：作者有话说（≤120字）+ 原创声明 + 赞赏作者 + 合集。
- 每章最终保存动作必须是“保存为草稿”。
...
- 标题生成后先读回标题框内容，发现 `undefined/NaN/空章号` 立即阻断，禁止带病保存。
- 正文与作者有话说必须在预处理阶段拆分完成；正文一旦检测到 `## 作者有话说`，该章必须回退重组。
- 赞赏与合集必须在保存前完成并可见，不得以“后续补设”替代。
...
- 每章均完成：标题、正文、作者有话说（≤120字）、原创声明、赞赏作者、合集、保存为草稿。
