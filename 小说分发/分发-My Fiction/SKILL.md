...
2. 若当前平台（My Fiction）匹配 `主输出平台` → 工作目录为 `小说正文/`
3. 若当前平台不匹配 `主输出平台`，或 `Agents.md` 不存在/未声明 `主输出平台` → 工作目录为 `My Fiction/`

后续所有章节文件路径、README.md、分发记录.md 均基于此工作目录。**明确说明**：
  - **本平台是主输出平台时** → 章节文件、`README.md`、`分发记录.md` 均在 `小说正文/` 下
  - **本平台不是主输出平台时** → 章节文件、`README.md`、`分发记录.md` 均在 `My Fiction/` 下

> **路径说明**：以下各节中所有 `My Fiction/` 形式的路经（如 `My Fiction/README.md`、`My Fiction/分发记录.md`）均为工作目录的示例写法。实际目录由上方「工作目录路由规则」确定：本平台是主输出平台时，所有这些路径对应 `小说正文/`；不是主输出平台时，对应 `My Fiction/`。
...
读取 `My Fiction/README.md` 后，必须检查其第一行一级标题是否等于 `# My Fiction 分发配置`：
  - **匹配** → 继续执行后续流程
  - **不匹配** → 立即停止上传，并向用户报错：`README.md 平台标识与当前 Skill 不符：预期"My Fiction 分发配置"，实际读取到"{实际读取到的标题}"。请检查是否将其他平台的 README.md 误放到了当前工作目录。`
### 3) 分发记录来源（强制）
