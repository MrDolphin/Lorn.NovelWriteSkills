...
2. 若当前平台（豆瓣）匹配 `主输出平台` → 工作目录为 `小说正文/`
3. 若当前平台不匹配 `主输出平台`，或 `Agents.md` 不存在/未声明 `主输出平台` → 工作目录为 `豆瓣/`

后续所有章节文件路径、README.md、分发记录.md 均基于此工作目录。**明确说明**：
  - **本平台是主输出平台时** → 章节文件、`README.md`、`分发记录.md` 均在 `小说正文/` 下
  - **本平台不是主输出平台时** → 章节文件、`README.md`、`分发记录.md` 均在 `豆瓣/` 下

> **路径说明**：以下各节中所有 `豆瓣/` 形式的路经（如 `豆瓣/README.md`、`豆瓣/分发记录.md`）均为工作目录的示例写法。实际目录由上方「工作目录路由规则」确定：本平台是主输出平台时，所有这些路径对应 `小说正文/`；不是主输出平台时，对应 `豆瓣/`。
...
1. **工具选择**：涉及数据通道（批量传正文/作者的话）时，先查阅 [六.1.F 数据通道提效](#f-数据通道提效可选)，确认已有 `scripts/content_parts_server.mjs` 是否覆盖当前需求；**禁止在已有工具可覆盖的情况下新建临时脚本**。
2. **弹窗判定**：编辑页出现浮层/弹窗时，先对照 [实操经验沉淀一：工具栏教学浮层 vs 真实安全验证](#2026-05-20-实跑修正如何区分工具栏教学浮层与真实安全验证) 做判断，再决定是否暂停；**禁止不查文档直接判定为安全验证**。
3. **限流/失败判定**：遇到"你发得太快"或保存异常时，先查 [实操经验沉淀二](#二创建与保存阶段的页面反馈) 和 [实操经验沉淀三](#三长批量分发策略) 对应小节，按已有策略处理。
4. **登录异常**：出现验证码/滑块/风控提示时，必须立即暂停并等待用户手动完成；在收到"登录完成"反馈前不得继续。
### 一、读取本地输入与模式判定
