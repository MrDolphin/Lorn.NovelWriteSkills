...
2. 若当前平台（小红书）匹配 `主输出平台` → 工作目录为 `小说正文/`
3. 若当前平台不匹配 `主输出平台`，或 `Agents.md` 不存在/未声明 `主输出平台` → 工作目录为 `小红书/`

后续所有章节文件路径、README.md、分发记录.md 均基于此工作目录。**明确说明**：
  - **本平台是主输出平台时** → 章节文件、`README.md`、`分发记录.md` 均在 `小说正文/` 下
  - **本平台不是主输出平台时** → 章节文件、`README.md`、`分发记录.md` 均在 `小红书/` 下

> **路径说明**：以下各节中所有 `小红书/` 形式的路经（如 `小红书/README.md`、`小红书/分发记录.md`）均为工作目录的示例写法。实际目录由上方「工作目录路由规则」确定：本平台是主输出平台时，所有这些路径对应 `小说正文/`；不是主输出平台时，对应 `小红书/`。
...
- 小红书最终上传时虽然使用绝对章节号口径，但绝对章节号只能基于最后一段章号再按本 Skill 的规则换算，不得把 `1.1.3`、`2.4.17` 这类三级编号原样带入标题输入框；最终只能写成 `第95章 三分钟` 一类格式。
...
读取 `小红书/README.md` 后，必须检查其第一行一级标题是否等于 `# 小红书分发配置`：
  - **匹配** → 继续执行后续流程
  - **不匹配** → 立即停止上传，并向用户报错：`README.md 平台标识与当前 Skill 不符：预期"小红书分发配置"，实际读取到"{实际读取到的标题}"。请检查是否将其他平台的 README.md 误放到了当前工作目录。`
### 3) 分发记录来源（强制）
...
- **先做本地预处理再进浏览器循环**：先统一提取 `标准标题/正文/内容指纹`，浏览器阶段只做“取数据→填写→暂存”，显著降低错章与漏填概率。
- **小批次推进（10章一批）**：分批执行可以在页面抖动、弹窗打断时快速定位影响范围，避免整批回滚。
- **固定节拍执行**：每章严格走“合集管理 → 添加长文笔记 → 写长文笔记 → 标题 → 正文 → 暂存离开”，减少定位器切换带来的不稳定。
- **台账及时落盘**：当批次完成后立即回写 `分发记录.md`，确保“新增/修改”判定和指纹比对有据可依。

### 暴露问题（教训）

- **标题提取存在边缘案例**：第36章源文件首行不是 `#` 标题格式，曾被错误生成为 `第36章 # 作者有话说`。
   - 教训：标题提取必须支持三类入口：
      1) `# 数字编号 标题`；
      2) `# 标题`；
      3) 无 `#` 时使用“首个非空行/文件名回退”策略。
- **批量过程中可能出现“暂无笔记标题”草稿**：说明某些草稿在保存时标题未正确写入或被中断。
   - 教训：每个小批次结束后必须执行一次草稿箱巡检，重点排查：
      - 是否存在 `暂无笔记标题`；
      - 是否存在 `第X章 # 作者有话说` 等异常标题；
      - 标题数量是否与计划章数一致。
- **“新增模式”易掩盖“修改模式”需求**：即使台账已有记录，草稿箱未命中同名时也会回退新增，可能产生重复草稿。
   - 教训：回退新增必须在备注里写明“台账存在但草稿未命中”，并在批次后清理重复项。

### 防错补丁（建议固化）

- 新增“**标题预检函数**”：保存前校验标题必须匹配 `^第\d+章\s.+`，不符合则禁止暂存。
- 新增“**批次对账三连**”：
   1) `计划章数 = 成功章数 + 失败章数`；
   2) 成功章都能在草稿箱命中标准标题；
   3) 台账记录数与成功章数一致。
- 新增“**异常草稿修复流程**”：发现异常标题后，按“草稿箱定位 → 编辑 → 正确标题/正文覆盖 → 暂存离开 → 台账标记修改”闭环处理。
