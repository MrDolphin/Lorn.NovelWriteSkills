...
| 工序 | 子技能/步骤 | 执行状态 | 关键结果 |
|:---:|--------|:----:|------|
| 1 | humanizer-zh | ✅/— | 清除 {N} 处 AI 信号（书面腔 {N}/过度总结 {N}/四字填充 {N}/收尾感 {N}） |
...
| 5 | 九大结构指纹诊断 | ✅/— | 触发指纹：{列出触发的指纹编号与名称，如无则写"无触发"} |
| 6 | 统计模式对抗（第九步） | ✅/— | 十维度扫描高危 TOP3：{维度名}；人类指纹注入：{已注入类型} |
| 7 | 三视角终审（第十步） | ✅ | 综合评级：🟢/🟡/🔴 |

> **工序状态**：✅ = 已执行；— = 未执行（快速通道模式跳过 / 该步骤不适用）
...
- 若走快速通道（四步法），工序 5（九大结构指纹诊断）和工序 6（统计模式对抗）标注为 —（跳过），但工序 7（三视角终审）在快速通道中由 shuorenhua 终审替代后仍需给出综合评级
- 修改记录至少列出 3 条实质性修改，可精简但不得为空。若本轮修改超过 10 条，选取最有代表性的 5–10 条记录
- 未修改项必须至少列出 1 条——证明去味时有明确的"不去碰"判断
- 综合评级采用第十步终审的三视角综合结论；快速通道模式采用 shuorenhua 终审结论 + stop-slop 五维评分综合判定
