...
**动作**：先跑 `去AI味红线保护.md` 做最终红线核查。再用 `去AI味后供血与职责复检卡.md` 复检卖点供血、章首抓力、中段回报、章末钩子与人物声音差。最后用 `去AI味执行清单.md` 做全局收尾，按 `去AI味多轮诊断与回写模板.md` 输出终稿。全流程完成后，按下方 `### 第十一步` 的去AI化报告模板输出归档报告。

**参考资料**：`去AI味红线保护.md`、`去AI味后供血与职责复检卡.md`、`去AI味执行清单.md`、`去AI味多轮诊断与回写模板.md`、`去AI味风格与排版补充.md`

**交付物**：`【修改总结与原因】—【修改后的小说正文】—【后续修改建议】` + 去AI化报告（全流程完成后）
...
**动作**：综合第八步复检数据、第九步统计对抗结果、第十步终审判定，生成一份结构化的去AI化报告。该报告与 `去AI味多轮诊断与回写模板.md` 的 `【修改总结与原因】—【修改后的小说正文】—【后续修改建议】` 格式为**互补关系**——诊断回写模板用于多轮交互中的每轮输出，去AI化报告用于全流程完成后的最终归档总结。

**交付物**：去AI化报告（Markdown 文件，格式见下方模板）

> **⚠️ 输出路径硬约束**：去AI化报告始终写入 `审阅意见/{卷目录}/`，**禁止写入 `小说正文/` 或其子目录**。小说正文目录只存放作品正文文件（含作者有话说与章节后记），去AI化报告是归档性审阅产物，不属于正文。文件名格式：`去AI化报告_{章节标识}_{日期}.md`，如 `去AI化报告_第1章_2026-07-21.md`。
...
| 检测维度 | 处理前 | 处理后 | 阈值 | 状态 |
|----------|:----:|:----:|:----:|:----:|
| 困惑度（低频词密度） | {N}/500字 | {N}/500字 | ≥1/500字 | ✅/⚠️ |
| 突发度（句长标准差） | {X}字 | {X}字 | ≥8字 | ✅/⚠️ |
| 连接词密度 | {N}/千字 | {N}/千字 | ≤1/千字 | ✅/⚠️ |
| 公式句数量 | {N}处 | {N}处 | 0处 | ✅/⚠️ |
| 段落规整度（段长标准差） | {X}句 | {X}句 | ≥2句 | ✅/⚠️ |
| 主语重复率 | {X}% | {X}% | <60% | ✅/⚠️ |
| 破折号异常率 | {X}% | {X}% | ≤1.5% | ✅/⚠️ |
| 感官词密度 | {N}/千字 | {N}/千字 | ≤2/千字 | ✅/⚠️ |
| 转折扣密度（不是…而是…） | {N}/千字 | {N}/千字 | ≤0.5/千字 | ✅/⚠️ |
| AI高频词密度 | {N}/千字 | {N}/千字 | ≤1/千字 | ✅/⚠️ |

> **判定规则**：✅ = 在正常范围内；⚠️ = 超出阈值，需关注但可能因题材/风格原因刻意保留。

---

## 工序执行记录

> **使用规则**：默认走全流程模式（双层记录），仅当用户明确要求"快一点"时走快速通道。两种模式按实际执行的路径择一填写，**不得混用，不得全填"—"敷衍**。

### 全流程模式（默认——表层+深层完整分层）

按实际执行顺序记入——先记表层4步（子技能），再记深层14步（十步流程）。所有层级均完整执行1–14轮；第九/十步为验证性工序——层级1改动少则扫描直接 Green 放行，层级2+若发现残留病灶则需记录定向对抗动作。
...
| 3 | 表层 | taste-skill（破模板增个性） | ✅/— | 声口/钩子/胆量检查：{通过/需修补} |
| 4 | 表层 | shuorenhua 场景化终审（public-writing） | ✅/— | 清理：模板感 {N} 处 / 表演腔 {N} 处 / 虚假主语 {N} 处 / 收束腔 {N} 处 |
...
| 8 | 深层 | 四·按档改写 | ✅/— | 执行动作：{公式句拆洗 / 解释腔压缩 / 句尾解释转断句 / ……}，共 {N} 处 |
...
| 13 | 深层 | 九·统计模式对抗（验证性） | ✅ | 十维度扫描所有维度均在正常范围；人类指纹注入：已确认（吐槽型OS冷幽默指纹 / 白描停顿节奏 / 沉默式关系温差） |
| 14 | 深层 | 十·人味编辑终审（验证性） | ✅ | 三视角综合：🟢 Green（签约编辑视角通过 / 老读者视角通过 / 检测工具视角通过） |

> **执行状态**：✅ = 已执行；— = 未启用。第九/十步为验证性工序，所有层级均执行——层级1扫描通过率通常为 Green，层级2+可能发现残留病灶需在修改记录中追加对抗动作。

### 快速通道（四步法——仅当用户明确要求"快一点"时使用）

> 快速通道跳过全部深层 references，仅以四步子技能快速收尾。适用于AI味较轻、用户要求快速交付的场景。
...
| 4 | shuorenhua 场景化终审 | ✅ | 场景=public-writing，档位=standard，清除 {N} 处模板感/{N} 处表演腔/{N} 处虚假主语
...
|------|:----:|--------------|--------------|------|
| 段1 L1-3 | 钩子强化 | "..." | "..." | 原开头以环境描写入场，抓力不足 |
| 段3 L2-5 | 模板腔拆洗 | "..." | "..." | "不是…而是…"结构 |
| 段4 | 感官均匀拆解 | "..." | "..." | 连续4句均为身体感官描写，删2句留白 |
| 段7 L1 | 毛边注入 | "..." | "..." | 连续3句长度均为15-18字，插入2字短句 |
| 段9 L4-6 | 解释腔拆洗 | "..." | "..." | "他意识到""这说明"等解释性引导词 |
| 章末段 | 钩子收紧 | "..." | "..." | 原章末用总结式收尾，改为认知缺口式钩子 |
| ... | ... | ... | ... | ... |

> **修改类型标签**：钩子强化 / 模板腔拆洗 / 解释腔拆洗 / 均匀句群打散 / 感官均匀拆解 / 毛边注入 / 视角修复 / 声口校准 / 雕琢腔拆除 / 钩子收紧 / 红线保护 / 读感优化
...
| 位置 | 内容摘要 | 保留原因 |
|------|------|----------|
| 段X L2 | {内容摘要} | 章末钩子——硬结构，只能提纯不能洗平 |
| 段Y | {内容摘要} | 人物声口指纹——该角色的标志性口癖 |
| 段Z L1-3 | {内容摘要} | 核心卖点句——控制卡标注的不可改项 |
| ... | ... | ... |
...
**使用规则**：
- 去AI化报告在**全流程完成后**输出，作为本章/本轮去味工作的归档总结
- 报告中的量化检测数据优先来自第九步的十维度扫描（从中提取与上表对应维度），若未执行第九步则在去味前后分别做简化的手动统计
...
- 未修改项必须至少列出 1 条——证明去味时有明确的"不去碰"判断
