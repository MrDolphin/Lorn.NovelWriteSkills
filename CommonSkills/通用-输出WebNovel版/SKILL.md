...
# 通用-输出WebNovel版
> **题材路由**：若 .github\题材专用Skills\ 目录存在对应的 <题材>-输出WebNovel版 Skill，则：
> - 将题材特性骨架路由到 <题材>-输出WebNovel版，该 Skill 位于 .github\题材专用Skills\ 目录。
> - 命中本技能时，必须优先强制加载当前 Skill 与 <题材>-输出WebNovel版。
...
若当前服务的项目根目录存在 `Agents.md`，执行WebNovel版输出前必须：

1. 读取项目根目录的 `Agents.md`
2. 若 `Agents.md` 中注册了三类模板，按以下优先级检索：
   - **优先**：模板注册时 "适用平台" 字段为 `WebNovel` 的同类型模板
   - **回退**：模板注册时 "适用平台" 字段为 `默认` 的同类型模板
   - **忽略**：模板注册时 "适用平台" 指向其他平台的模板（如 `GoodNovel`、`My Fiction`），本轮不加载
3. 若检索到匹配的三类模板——读取对应路径的模板文件：
   - 写作研究模板：作为WebNovel平台的额外平台基线约束
   - 作者风格模板：作为WebNovel版本保留底味和文风边界的参照
   - 作品蓝本模板：作为章首/回报/钩子结构保真的参照
4. 若项目根目录不存在 `Agents.md`，或其中未注册WebNovel专属模板——回退通用默认模式，不影响正常输出
5. 本 Skill 的核心约束始终是：平台规则 > 安全合规 > 母稿事实 > 三模板约束。三模板是辅助，不改变平台输出规则

## 默认执行顺序

1. 先锁定源稿不可变层：事实、事件链、证据链、角色关系与因果顺序不得改坏。
2. 读取对应题材的 `题材名-输出WebNovel版` Skill，确认题材入口与路由关系。
...
6. 最后才处理降相似度，不得为了降重破坏英文可读性与平台节奏。
...
### WebNovel 版的人称适用场景（供创建/修订基线表条目时参考）

**平台底层逻辑**：WebNovel 是全球读者量最大的英文网文平台，读者跨越数十种文化背景。跨文化可读性是一切的前提。研究建议多线叙事，每 2-3 章可切换视角。

**为什么 WebNovel 偏第三人称？**
- **跨文化可读性是 WebNovel 的第一约束。** 第一人称中的文化特定内心活动（如"我心里咯噔一下""一股说不清道不明的滋味涌上心头"）对非母语读者构成理解障碍。第三人称的"他看到了/他闻到了/他意识到"这种直白因果链，全球读者都能跟上。
- 第三人称的信息差结构（"读者与主角同步获知 + 偶尔切换视角制造读者知道但主角不知道的紧张"）是全球通用的追更驱动机制。第一人称需要更强的文化语境共鸣才能达到同等效果。
- 研究建议的多线叙事（每 2-3 章切换视角）在第三人称下最自然——不同视角章节之间的切换只需要一个"POV: [Character Name]"标记，不会造成叙述断裂。

**什么时候 WebNovel 可以用第一人称？**
- romance 类型——情感是人类共通语言，第一人称的情绪连接不需要跨文化解码。
- WebNovel 上第一人称 romance/thriller 已有成熟受众群，情绪连接优势明确。
- **警告**：不可突然混入第三人称叙述。全书统一人称是底线。
...
- WebNovel 平台章节标题纯标题长度不得超过 **70 个字符**；但不应机械压短，应优先服从平台跨文化可读性与信息清晰度，在不超限前提下保持标题表达完整。
