...
## 场景GTK循环——每个场景的铁律（新增——吸收自知乎雪花写作法AI应用）

> **来源**：知乎·芒果留了果《AI写小说水文破解：用雪花写作法重构长篇创作逻辑》（2026-07-29）。

雪花写作法的场景层铁律：**每一个场景必须包含"目标-冲突-挫折"的循环。没有冲突的场景直接删掉。**

### GTK三要素

| 要素 | 定义 | 缺失表现 |
| --- | --- | --- |
| **目标（Goal）** | 场景开始时，POV角色明确想要达成什么——一个具体的、场景级的微型目标 | 角色在场景中被动漂移，读者不知道"这场戏在争什么" |
| **冲突（Tension/Konflikt）** | 有什么在阻碍角色达成目标——对手、环境、规则、信息差、自身局限 | 角色一路顺风顺水达成目标，场景变成"展示"而非"拉扯" |
| **挫折（Knockback）** | 场景结束时，角色未能完全达成目标，或达成了但有代价/新问题/意外后果 | 场景收在"搞定了"的舒适区，没有把压力传递到下一场 |

### GTK与爽点公式的关系

本 Skill 已有的"爽点四步公式"（产生诉求→给予希望→努力解决→得偿所愿）是**章级/卷级的宏观爽感曲线**，而 GTK 是**场景级的微观驱动单元**。二者是分层协作关系：

- 一个"得偿所愿"的爽点章，内部可以由多个 GTK 场景组成——主角在达成最终目标前经历了多次小挫折
- 一个以"挫折"收尾的 GTK 场景，恰好在为下一章的"得偿所愿"蓄力
- 如果一个场景只有"目标→达成"而没有中间的冲突与挫折，它就是"无效场景"——即使放在爽点章里，也会稀释爽感浓度

### 施工卡中的GTK标注要求

- 控制卡/施工卡中每个场景单元必须标注：**G（目标）、T（冲突/障碍）、K（挫折/退场状态）**
- 若某个场景无法填写 GTK 三项中的任一项 → 该场景功能不成立，应删除或与相邻场景合并
- 场景退场状态（K）必须与下一场景的入场状态（G）形成因果链："因为上一场没达成 X，所以这一场必须面对 Y"

### GTK快速诊断

写完场景后，用三问快速自检：
1. 这个场景里，POV角色**想要什么**？（能一句话说清 → G 成立）
2. 他想要的东西**遇到了什么阻力**？（有具体阻碍，不是"命运"这种抽象物 → T 成立）
3. 场景结束时，他**拿到想要的东西了吗**？如果拿到了——他付出了什么代价？如果没拿到——他失去了什么或发现了什么？（有代价/有失去/有新发现 → K 成立）

三问中任一答案为"说不清"，该场景需要重写或删除。
...
- 若项目根注册了"作者风格进化"数据源（见 `Agents.md`）：
  - **执行中（只读，不提问）**：读取 `作者风格画像.md` 的 叙事方法层 + 情绪审美层 + 人物声口库 作为场景作者约束（优先级：作者画像 > 作者风格模板 > 通用默认基线）——场景铺法（直给 / 留白 / 推进方式）按叙事方法层指纹写，不像作者"怎么铺一个场景"就算执行偏移
