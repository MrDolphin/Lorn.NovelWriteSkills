...
# 通用-输出出版社版
> **题材路由**：若 .github\题材专用Skills\ 目录存在对应的 <题材>-输出出版社版 Skill，则：
> - 将题材特性骨架路由到 <题材>-输出出版社版，该 Skill 位于 .github\题材专用Skills\ 目录。
> - 命中本技能时，必须优先强制加载当前 Skill 与 <题材>-输出出版社版。
...
若当前服务的项目根目录存在 `Agents.md`，执行出版社版输出前必须：

1. 读取项目根目录的 `Agents.md`
2. 若 `Agents.md` 中注册了三类模板，按以下优先级检索：
   - **优先**：模板注册时 "适用平台" 字段为 `出版社` 的同类型模板
   - **回退**：模板注册时 "适用平台" 字段为 `默认` 的同类型模板
   - **忽略**：模板注册时 "适用平台" 指向其他平台的模板（如 `番茄`、`起点`），本轮不加载
3. 若检索到匹配的三类模板——读取对应路径的模板文件：
   - 写作研究模板：作为出版社平台的额外平台基线约束
   - 作者风格模板：作为出版社版本保留底味和文风边界的参照
   - 作品蓝本模板：作为章首/回报/钩子结构保真的参照
4. 若项目根目录不存在 `Agents.md`，或其中未注册出版社专属模板——回退通用默认模式，不影响正常输出
...
### 出版社版的人称适用场景（供创建/修订基线表条目时参考）

**平台底层逻辑**：传统出版社的核心关注不是阅读量/完读率，而是**编辑审读通过率**。编辑看稿时在意的三个东西：结构是否完整、专业细节是否可信、叙事声音是否稳定。人称选择必须服务这三件事。

研究显示出版社都市悬疑存在两条成熟路径：
- **路径一（法医秦明模式）**：第一人称职业视角——通过专业身份（法医/犯罪心理侧写师/痕检员）锚定叙事可信度。主角的"我"不是普通人的"我"，而是"我作为法医，看到了……"的专业视角。这种模式的核心逻辑是：第一人称 + 职业壁垒 = 读者既代入又仰望。
- **路径二（雷米模式）**：有限第三人称职业视角——通过单一核心角色的职业视角展开叙事（如《心理罪》中方木的视角），严格限制其他角色的内心活动暴露。结构更稳健，适合多卷本长线叙事。

**判断标准——回答这个问题**："故事的核心吸引力来自'一个专业人员的亲口讲述'，还是'一个专业世界的客观呈现'？"
- 前者→第一人称职业视角。法医/侧写师/痕检员说"我看到的……"，说服力远强于第三人称。
- 后者→第三人称职业视角。全景式群像叙述，结构更稳健，审读更友好。
- **两者都绝对禁止全知视角**。编辑对"上帝视角补充信息"的容忍度为零——它被视为"作者偷懒"。
...
- 出版社平台章节标题纯标题长度不得超过 **20 个字**；但限制长度不代表越短越好，应优先保证出版审读语境下的准确性与完整性，在不超限前提下保留必要信息。
