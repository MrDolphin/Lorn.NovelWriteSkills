...
# 通用-设计故事面
> **题材路由**：若 .github\题材专用Skills\ 目录存在对应的 <题材>-设计故事面 Skill，则：
> - 将题材特性骨架路由到 <题材>-设计故事面，该 Skill 位于 .github\题材专用Skills\ 目录。
> - 命中本技能时，必须优先强制加载当前 Skill 与 <题材>-设计故事面。
...
## ★ 叙事传递链——故事面要向下级"讲故事"（强制）

> 故事面是全线大纲体系中最擅长"讲故事"的层级——它有"一句话谜面"和"故事梗概"。但当前的问题在于：**故事面讲完的故事，在传递到分卷和逐章时被翻译成了工程字段（章目标、核心冲突、关键情节），丢失了叙事可读性。** 本节强制要求故事面的叙事必须完整、可辨认地传递到下级。

### 传递链要求

1. **故事面的"一句话谜面"必须被分卷的"卷故事概要"继承**：分卷设计时，对应本面承接的卷必须把"一句话谜面"翻译为该卷的叙事子问题——用自然语言，不是用工程字段。
2. **故事面的"故事梗概"（4B 卷内完整故事）必须可被逐章微型剧本的"▶ 本章在讲什么"识别**：逐章设计时，本章在故事面的叙事弧中处于什么位置、承担什么功能——必须在这个字段中显式写出。
3. **验收标准**：把故事面文件、分卷大纲、逐章微型剧本按顺序给一个人看——他应该能追踪到同一个故事从"面"到"卷"到"章"的完整叙事流。如果在某级断裂（比如分卷读完不知道故事面在这里推进了什么），判为叙事传递失败。

### 传递失败处理

- 若分卷的"卷故事概要"与故事面的"一句话谜面"对不上 → 回到故事面层调整，或明确本卷不服务于该面
- 若逐章的"▶ 本章在讲什么"与故事面的"故事梗概"对不上 → 回到故事面或分卷层对齐
- **不得为了对齐而把工程字段改写为叙事语言**——工程字段的存在理由是执行精度，叙事字段的存在理由是故事理解。两者不可互相替代。
...
- 若项目根存在 `Agents.md` 且其中注册了"作者风格进化"数据源（`蒸馏产物/作者风格进化/` 下的 作者风格画像.md 与 作者原声样本库.md）：
  - **执行中（只读）**：读取 `作者风格画像.md` 的 价值母题层 + 情节伦理显影层 作为故事面作者约束（优先级：作者画像 > 作者风格模板 > 通用默认基线）——一句话谜面应落在作者最关心的母题上，硬抓手不宜踩作者反感的价值底线
...
## 故事面的三问验收（新增——吸收自知乎桐梦书涵·三公理）

每个故事面设计完成后，用以下三个问题做最终验收。这三个问题比任何写作教程都值钱：

```text
1. 读者会停吗？   （注意力公理：这一面的第一个故事入口是否有"未完成事件+可立即共情"？）
2. 读者会猜吗？   （预期差公理：这一面的爽感/虐感是否建立在"先钉预期再打碎"上？）
3. 读者会在乎吗？（情绪投资公理：读者是否已对这一面的核心人物做了情绪存款？）
```

> **来源**：桐梦书涵《写小说必须遵循黄金前三章？》（2026-09-02）；详见 `CommonSkills/通用-强化章节开头/references/黄金三章底层三公理.md`

### 验收与既有框架的协同

| 三问 | 对应故事面框架 | 验收标准 |
|------|--------------|---------|
| 读者会停吗？ | 核心驱动力（某人→渴望→遇阻→改变） | 第一个入口事件是否能 3 秒内让读者感知到"谁在承压" |
| 读者会猜吗？ | 预期差设计（四层矛盾网络） | 矛盾冲突是否建立在"先建立读者预期→再偏离预期"的模式上 |
| 读者会在乎吗？ | 情绪投资（人物弧线 + 站队设计） | 核心人物是否有至少一个"读者愿意站队"的共情锚点 |
...
固定优先级：**先确认故事讲清楚了，再定定位与谜面，再定六要素与卷级供血，最后落 4A / 4B、供血锚点与检查清单。**

0. **★ 叙事自检门禁（新增—最高优先级）**：在进入任何工程流程之前，先用自然语言回答三个问题：
   - 这个故事面"在讲一个什么故事"？尝试用一句话说清（不是卖点，是故事）
   - 如果有人问"这条支线删掉会影响全书吗"，你的回答是"会，因为____"吗？
   - 这个故事面的叙事与总纲的"一句话讲故事"方向一致吗？
   - **如果连"这条支线在讲什么故事"都说不出一句让人能复述的话——不推进到任何工程字段。先回去写故事本身。**
...
### 叙事层（新增—优先检查）
- [ ] **能一句话说清"这个故事面在讲什么故事"**——不是卖点、不是套路名、不是一个抽象主题
- [ ] **故事面的故事可被分卷继承**：把一个不熟悉本项目的人从故事面文件带到对应分卷，他能识别出"哦，这就是故事面里说的那部分"
- [ ] **逐章的人也能说清"这章在故事面中处于什么位置"**
